...
export const Route = createFileRoute("/resources")({
  head: () => ({
    meta: [
      { title: "Resources — Ondo State Sustainable Cocoa MSP" },
      {
        name: "description",
        content:
          "Download meeting reports, policy documents, training manuals and research publications from the Ondo State Sustainable Cocoa MSP.",
      },
      { property: "og:title", content: "MSP Resource Library" },
      {
        property: "og:description",
        content: "Reports, policies, training manuals and research publications.",
      },
    ],
  }),
  ...
  errorComponent: ({ error }) => (
    <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      <p className="text-sm text-destructive">Couldn't load resources: {getErrorMessage(error)}</p>
    </div>
  ),
  pendingComponent: () => (
...
function ResourcesPage() {
  ...
  const byCategory = filter === "all" ? resources : resources.filter((r) => r.category === filter);
    ...
    <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      ...
      {visible.length === 0 ? (
        <div className="mt-10 rounded-2xl border border-dashed border-border bg-card/50 p-12 text-center text-sm text-muted-foreground">
          {q ? "No resources match your search." : "No resources in this category yet."}
        </div>
      ) : (
